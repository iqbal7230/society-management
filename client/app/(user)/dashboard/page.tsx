    ...
    <div>
      ...
      {records.length === 0 && myFlat && myPlan && (
        <div className="mb-6 bg-bg-card border border-border-active rounded-xl p-5">
          ...
          <p className="text-xs text-text-secondary mt-2">
            Flat {myFlat.flat_no} · {myFlat.type}. You will receive notifications and payment
            reminders for this flat here.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        <div className="bg-bg-card border border-border-default rounded-xl p-5">
          <p className="text-text-muted text-sm font-medium mb-1">This month ({getMonthLabel(currentMonth)})</p>
          <p className={`text-xl font-bold ${currentRecord?.status === "paid" ? "text-success" : "text-warning"}`}>
            {currentRecord?.status === "paid" ? "Paid" : "Pending"}
          </p>
          {currentRecord && (
            <p className="text-sm text-text-secondary mt-1">
              ₹{Number(currentRecord.amount).toLocaleString("en-IN")}
              {currentRecord.status === "paid" && currentRecord.payment_mode && (
        ...
        <div className="bg-bg-card border border-border-default rounded-xl p-5">
          ...
          <p className="text-xs text-text-muted mt-1">{pendingRecords.length} month(s) pending</p>
        </div>
        <div className="bg-bg-card border border-border-default rounded-xl p-5">
          <p className="text-text-muted text-sm font-medium mb-1">Quick actions</p>
          <Link
            href="/pay-now"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <h2 className="text-lg font-semibold text-text-primary mb-3">Payment history</h2>
              ...
              <ul className="divide-y divide-border-default">
                {recentRecords.map((r) => (
                  <li key={r.id} className="flex items-center justify-between px-4 py-3">
                    <span className="text-text-primary">{getMonthLabel(r.month)}</span>
                    <span className="text-text-secondary">₹{Number(r.amount).toLocaleString("en-IN")}</span>
                    <span
              ...
              <ul className="divide-y divide-border-default">
                {notifications.slice(0, 5).map((n) => (
                  <li key={n.id} className="p-4">
                    ...
                    <p className="text-xs text-text-muted mt-2">{n.date}</p>
                  </li>
                ))}
              </ul>
            )}
